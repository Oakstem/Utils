...
def check_rsa_fmri_against_models(fmri_rdms, model_rdms):
    rsa_results = {}
    for fmri_name, fmri_rdm in fmri_rdms.items():
        for model_name, model_rdm in model_rdms.items():
            fmri_rdm, model_rdm = fix_uneven_dataframes(fmri_rdm, model_rdm)
            rsa_corr = create_rsa_from_two_rdm_mats(fmri_rdm, model_rdm, fmri_name, model_name,
                                                    'rsa', 'full_corr_rsa.csv',
                                                    'partial_corr_rsa.csv')
            rsa_results[f"{fmri_name}/{model_name}"] = rsa_corr[0]['r'].values[0]
    return rsa_results
